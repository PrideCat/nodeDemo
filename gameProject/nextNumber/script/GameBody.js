...
GameBody.prototype.gameOver = function () {
    ...
    continueBtn.x = LGlobal.width * (.5 - 0.0);
    ...
    self.more.addEventListener(LMouseEvent.MOUSE_DOWN, self.onMoreBtnDown);

    self.share(self.numberFound, gScore);
}
GameBody.prototype.share = function (number, score) {
    var self = this;
    var shareTitle = "数到了" + number + "，获得" + score + "分，感觉数学白学了";
    if (number >= self.targetNumberFound) {
        shareTitle = "终于数完了" + self.targetNumberFound + "，获得" + score + "分，数数也不容易！";
    }
    btGame.setShare({
        title: shareTitle
    });
    var msg = ""
    if (number < 15) {
        msg = "你找到了" + number + "个数字，获得" + score + "分，你的数学是体育老师教的吗？分享游戏，让朋友也来挑战一下吧！"
    } else if (number < 30) {
        msg = "你找到了" + number + "个数字，获得" + score + "分，数学水平一般！分享游戏，让朋友也来挑战一下吧！"
    } else {
        msg = "你找到了" + number + "个数字，获得" + score + "分，数学水平超神啦！分享游戏，让朋友也来挑战一下吧！";
    }
    setTimeout(function () {
        btGame.playScoreMsg(msg);
    }, 500);
}
...
GameBody.prototype.onMoreBtnDown = function (event) {
    try {
        var directions = plus.webview.create(btGame.URL.getMoreGame(), "directions");
